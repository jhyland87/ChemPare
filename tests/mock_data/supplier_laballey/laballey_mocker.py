...
class curl_cffi:
    ...
    def post(path, *args, **kwargs):
        if not path.endswith('/getresults') or 'json' not in kwargs or "searches" not in kwargs["json"]:
            return None
        ...
        mock_response_file = f"supplier_laballey/response_data/POST__getresults__{fname}.json"

        return CurlRequestMocker(file=mock_response_file)

    @staticmethod
    def get(path, *args, **kwargs):
        if not path.endswith('/getresults') or "params" not in kwargs or "q" not in kwargs["params"]:
            return None
        ...
        mock_response_file = f"supplier_laballey/response_data/GET__getresults__{fname}.json"
