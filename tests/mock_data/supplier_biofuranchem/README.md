# Biofuran Chem Mock Data

The mock data was generated by using the below bash scripts:


## Get the session binding from the initial request headers
```bash
curl 'https://www.biofuranchem.com' \
    --get --silent --head \
    --request-target "/shop" \
    --data "Category=All" \
...
## All together in a single bash script:
```bash
#!/usr/bin/env bash
